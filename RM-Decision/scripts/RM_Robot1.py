...
class IsClose(Task):
    ...
    def run(self):
        if env.EnemyPoseSave_robot1.enemy_dist < 1.58 and env.EnemyPoseSave_robot1.enemy_dist > 0 and self.blackboard.Blocked_Prevent_close == False:
        ...
        if env.EnemyPoseSave_robot1.enemy_dist > 0.8:  # 否则后退一些
            if self.blackboard.sended_close == False:
                ...
                if env.isActionAvaliable(self.goal_x, self.goal_y, self.goal_yaw):  # 指向敌人
                    ...
                    print 'r1： turn to enemy!!!!!!!!!!1 goal is, %s, %s' % (env.EnemyPoseSave_robot1.enemy_yaw, self.goal_yaw)
                else:
            ...
            if self.blackboard.sended_close == False:
                controller.send_vel_robot1(cmdvel_slowback)
                self.blackboard.sended_close = True
                self.time1 = rospy.Time.now().secs
                print 'r1: retreat!!!!!!!!!!!!!!!!%s, %s, %s' % (self.goal_x, self.goal_y, self.goal_yaw)
            if rospy.Time.now().secs - self.time1 > 0.2:
                self.blackboard.sended_close = False
                controller.send_vel_robot1(cmdvel_stop)
                controller.send_vel_robot1(cmdvel_stop)
                print 'retreat end!!!!!!!!!!!!!!!'
        return TaskStatus.SUCCESS
...
class IsFar(Task):
    ...
    def run(self):
        if env.EnemyPoseSave_robot1.enemy_dist > 1.58 and self.blackboard.Blocked_Prevent_far == False:
            self.blackboard.sended_close = False
            self.blackboard.sended_beated = False
            self.blackboard.sended_patrol = False
            self.blackboard.sended_follow = False
    ...
    def run(self):
        ...
        if self.blackboard.sended_far == False:
            if env.isActionAvaliable(env.goal_x_robot1, env.goal_y_robot1, env.goal_yaw_robot1):
                ...
                self.goal_y = env.goal_y_robot1
                self.goal_yaw = env.goal_yaw_robot1
            else:
                self.goal_x = env.farchoosegoal_x_robot1
                self.goal_y = env.farchoosegoal_y_robot1
                self.goal_yaw = env.farchoosegoal_yaw_robot1
            ...
            if env.isActionAvaliable(self.goal_x, self.goal_y, self.goal_yaw):  # 指向敌人
                ...
                print 'r1: Far track!!!!!!!!!!!!!! goal is %s,%s,%s ;enemy is in %s, %s ' % (self.goal_x, self.goal_y, self.goal_yaw,env.EnemyPoseSave_robot1.enemy_dist,env.EnemyPoseSave_robot1.enemy_yaw)
            else:
                self.time1 = rospy.Time.now().secs
...
class BeatedShoot(Task):
    ...
    def run(self):
        if self.blackboard.sended_beated == False:
            self.goal_x = env.MyPose_robot1['x']
            self.goal_y = env.MyPose_robot1['y']
            if env.witch_armor_robot1 == 0:  # 主 beated
                if env.EnemyPoseSave_last_robot1 >= 0:
                    self.goal_yaw = env.MyPose_robot1['theta'] + 30
                else:
                    self.goal_yaw = env.MyPose_robot1['theta'] - 30
            elif env.witch_armor_robot1 == 1:  # 左
                self.goal_yaw = env.MyPose_robot1['theta'] + 120
            elif env.witch_armor_robot1 == 2:  # 后
                self.goal_yaw = env.MyPose_robot1['theta'] + 178
...
class IsFollow(Task):
    ...
    def run(self):
        if env.EnemyPoseSave_robot1.enemy_dist == 0 and env.EnemyPoseSave.enemy_dist > 0 and self.blackboard.Blocked_Prevent_follow == False:
            self.blackboard.sended_far = False
            self.blackboard.sended_close = False
            self.blackboard.sended_beated = False
            self.blackboard.sended_patrol = False
    ...
    def run(self):
        if self.blackboard.follow_flag == 0:
            if self.blackboard.sended_follow == False:
                self.goal_x = env.choosedgoal_x_robot1
                self.goal_y = env.choosedgoal_y_robot1
                self.goal_yaw = env.choosedgoal_yaw_robot1
                self.blackboard.BlockedPoseSaveX = env.MyPose_robot1['x']
                self.blackboard.BlockedPoseSaveY = env.MyPose_robot1['y']
                self.blackboard.BlockedPoseSaveYaw = env.MyPose_robot1['theta']
                if env.isActionAvaliable(self.goal_x, self.goal_y, self.goal_yaw):  # 指向敌人
                    controller.send_goal_robot1(env.navgoal)
                    self.time1 = rospy.Time.now().secs
                    print 'r1: is follow !!!!!!!!!!!!!!%s,%s,%s' % (self.goal_x,self.goal_y,self.goal_yaw)
                    self.blackboard.sended_follow = True
                else:
                    self.time1 = rospy.Time.now().secs
                    print 'r1: cant follow %s, %s, %s' % (self.goal_x, self.goal_y, self.goal_yaw)
            if rospy.Time.now().secs - self.time1 > 2:
                ...
                if np.square(self.blackboard.BlockedPoseSaveX - env.MyPose_robot1['x']) + np.square(self.blackboard.BlockedPoseSaveY - env.MyPose_robot1['y']) < 0.0225 and np.abs(self.blackboard.BlockedPoseSaveYaw-env.MyPose_robot1['theta']) < 10:
                    ...
                    self.blackboard.sended_follow = False
                    if np.square(self.goal_x - env.MyPose_robot1['x']) + np.square(self.goal_y - env.MyPose_robot1['y']) < 0.01:  #到了
            ...
            if self.blackboard.sended_follow == False:
                self.goal_x = env.choosedgoal_x_robot1
                self.goal_y = env.choosedgoal_y_robot1
                self.goal_yaw = env.choosedgoal_yaw_robot1 + 30
                if self.goal_yaw > 180:
                    self.goal_yaw = self.goal_yaw - 360
        ...
        if self.blackboard.patrol_flag1 == 0:  #先自转
            ...
            if self.blackboard.sended_patrol == False:
                if env.isActionAvaliable(self.goal_x, self.goal_y, self.goal_yaw):  # front1
                    controller.send_goal_robot1(env.navgoal)
                    print 'r1 is patrol_spin; Now Pose is %s, %s, %s' % (env.MyPose_robot1['x'], env.MyPose_robot1['y'], env.MyPose_robot1['theta'])
                    self.time1 = rospy.Time.now().secs
                    self.blackboard.sended_patrol = True
            else:
                if rospy.Time.now().secs - self.time1 > 2.5 or np.abs(env.MyPose_robot1['theta'] - self.goal_yaw) < 10:
                    self.blackboard.sended_patrol = False
            ...
            if self.blackboard.patrol_flag == 0:  # 第一点：移动
                if self.blackboard.sended_patrol == False:
                    ...
                    if env.isActionAvaliable(self.goal_x, self.goal_y, self.goal_yaw):  # front1
                        controller.send_goal_robot1(env.navgoal)
                        print 'r1 is patrol_front; goal is %s, %s, %s' % (self.goal_x, self.goal_y, self.goal_yaw)
                        self.time1 = rospy.Time.now().secs
                        self.blackboard.sended_patrol = True
                else:
                    if rospy.Time.now().secs - self.time1 > 2.5:
                ...
                if self.blackboard.sended_patrol == False:
                    ...
                    if env.isActionAvaliable(self.goal_x, self.goal_y, self.goal_yaw):  # front1
                        controller.send_goal_robot1(env.navgoal)
                        print 'r1 is patrol_front1; goal is %s, %s, %s' % (self.goal_x, self.goal_y, self.goal_yaw)
                        self.time1 = rospy.Time.now().secs
                        self.blackboard.sended_patrol = True
                else:
                ...
                if self.blackboard.sended_patrol == False:
                    ...
                    if env.isActionAvaliable(self.goal_x, self.goal_y, self.goal_yaw):  # front1
                        controller.send_goal_robot1(env.navgoal)
                        print 'r1 is patrol_front2; goal is %s, %s, %s' % (self.goal_x, self.goal_y, self.goal_yaw)
                        self.time1 = rospy.Time.now().secs
                        self.blackboard.sended_patrol = True
                else:
                    if rospy.Time.now().secs - self.time1 > 3:
                ...
                if self.blackboard.sended_patrol == False:
                    ...
                    if env.isActionAvaliable(self.goal_x, self.goal_y, self.goal_yaw):  # front1
                        controller.send_goal_robot1(env.navgoal)
                        print 'r1 is patrol_back1; goal is %s, %s, %s' % (self.goal_x, self.goal_y, self.goal_yaw)
                        self.time1 = rospy.Time.now().secs
                        self.blackboard.sended_patrol = True
                else:
                    if rospy.Time.now().secs - self.time1 > 2:
                ...
                if self.blackboard.sended_patrol == False:
                    ...
                    if env.isActionAvaliable(self.goal_x, self.goal_y, self.goal_yaw):  # front1
                        controller.send_goal_robot1(env.navgoal)
                        print 'r1 is patrol_back2; goal is %s, %s, %s' % (
        ...
        if self.blackboard.Blocked_excuteflag == 0:  #转正
            if self.blackboard.sended_blocked == False:
                ...
                for k in range(0, len(self.abstheta)):
                    del self.abstheta[0]
                for k in range(0, len(env.blockedthetas_robot1)):
                    self.abstheta.append(abs(self.blackboard.Blocked_GoalYaw-env.blockedthetas_robot1[k]))
                    print env.blockedthetas_robot1
                    print self.abstheta
                self.goal_yaw = env.blockedthetas_robot1[self.abstheta.index(min(self.abstheta))]
                if np.abs(env.MyPose_robot1['theta']-(self.goal_yaw+30)) < np.abs(env.MyPose_robot1['theta']-(self.goal_yaw-30)):  #右转
                    self.goal_yaw = self.goal_yaw + 30
                    cmdvel_blockedpush.linear.x = 0.2*np.cos(30*3.1416/180)
                    ...
                    cmdvel_blockedretreat.linear.x = -0.2 * np.cos(30*3.1416/180)
                    cmdvel_blockedretreat.linear.y = 0.2 * np.sin(30*3.1416/180)
            ...
            if (rospy.Time.now().secs - self.time1 > 5) or (np.abs(env.MyPose_robot1['theta'] - self.goal_yaw) < 5):  # 可以下一步啦
            ...
            if rospy.Time.now().secs - self.time1 < 3:
                controller.send_vel_robot1(cmdvel_blockedpush)
            else:
                controller.send_vel_robot1(cmdvel_stop)
                controller.send_vel_robot1(cmdvel_stop)
                print 'qiantui Done!!!!!!!'
                if np.square(env.MyPose_robot1['x']-self.blackboard.BlockedPoseSaveX)+np.square(env.MyPose_robot1['y']-self.blackboard.BlockedPoseSaveY) > 0.04 and self.blackboard.avoidloop_far < 3 and self.blackboard.avoidloop_follow < 3 and self.blackboard.avoidloop_patrol < 5:  #前推成功
            ...
            if rospy.Time.now().secs - self.time1 < 2:
                controller.send_vel_robot1(cmdvel_blockedretreat)
            else:
                controller.send_vel_robot1(cmdvel_stop)
                controller.send_vel_robot1(cmdvel_stop)
            ...
            if self.blackboard.sended_blocked == False:
                ...
                for k in enumerate(self.blackboard.PreRescuePoint):
                    self.distfromeme.append(np.square(env.MyPose_robot1['x']-k[1][0])+np.square(env.MyPose_robot1['y']-k[1][1]))
                ...
                if env.isActionAvaliable(self.goal_x, self.goal_y, self.goal_yaw):  # 指向敌人
                    ...
                    self.blackboard.sended_blocked = True
                    self.time1 = rospy.Time.now().secs
            if (rospy.Time.now().secs - self.time1 > 4) or (np.square(env.MyPose_robot1['x'] - self.goal_x)+np.square(env.MyPose_robot1['y']-self.goal_y) < 0.09):  #执行完啦
